...
class UserAmisService {
  static async createRequestAmi(
    ...
    try {
      ...
      throw ApiError.internal('Erreur inconnue dans createRequestAmi');
    }
  }
  ...
  static async getAllUserAmis({
    page,
    perPage,
  }: {
    page: number;
    perPage: number;
  }): Promise<{ userAmis: UserAmis[]; total: number }> {
    try {
      ...
      if (userAmis.length === 0) {
        throw ApiError.notFound(
          "Il n'y actuellement aucune demande de petsitting dans la db"
        );
      }
      ...
      throw ApiError.internal('Erreur inconnue dans getAllUserAmis');
    }
  }

  static async getAllUserAmisForAUser({
    userId,
    page,
    perPage,
  }: {
    ...
    try {
      ...
      throw ApiError.internal('Erreur inconnue dans getAllUserAmisForAUser');
    }
  }

  static async getAllUserAmisForAPetsitter({
    currentuserid,
    page,
    perPage,
  }: {
  ...
  static async ResponseToFriendRequest(
    ...
    try {
      ...
      throw ApiError.internal('Erreur inconnue dans ResponseToFriendRequest');
    }
  }

  static async updateDemandeAmiWithAllParameters(
    id: string,
    data: Partial<UserAmis>
  ): Promise<[boolean, UserAmis | null]> {
    try {
      if (!id) {
      ...
      throw ApiError.internal('Erreur inconnue dans updateDemandeAmi');
    }
  }
}
